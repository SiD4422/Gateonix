...
mainCanvas.addEventListener('mousemove', e=>{
  ...
  if(dragging){
    ...
    if(!_didDrag && _dragStartPos){
      ...
      if(dist < DRAG_THRESHOLD / view.zoom) return; // not moved enough yet
    }
...
mainCanvas.addEventListener('wheel', e=>{
  ...
  const nz=Math.max(0.2,Math.min(3.5,view.zoom*delta));
  view.panX=mx-(mx-view.panX)*(nz/view.zoom);
  view.panY=my-(my-view.panY)*(nz/view.zoom);
  view.zoom=nz; zLabel.textContent=Math.round(nz*100)+'%'; render();
},{passive:false});
