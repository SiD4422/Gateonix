...
window.CodePanel = (function () {
  ...
  function runTruthTable(){
    ...
    var saved=inputs.map(function(g){return g.type==='CLOCK' ? (g.clockVal??0) : (g.value??0);});
    ...
    // Save current state of ALL components to evaluate each row independently from current live state
    var stateSnapshot = gs.map(function(g){
      return { q: g.q, nq: g.nq, prevClk: g.prevClk, clockVal: g.clockVal };
    });

    for(var mask=0;mask<rows;mask++){
      // Restore state
      gs.forEach(function(g, i){
        if (stateSnapshot[i].q !== undefined) g.q = stateSnapshot[i].q;
        if (stateSnapshot[i].nq !== undefined) g.nq = stateSnapshot[i].nq;
        if (stateSnapshot[i].prevClk !== undefined) g.prevClk = stateSnapshot[i].prevClk;
        if (stateSnapshot[i].clockVal !== undefined) g.clockVal = stateSnapshot[i].clockVal;
      });

      var inVals=Array.from({length:n},function(_,b){return (mask>>(n-1-b))&1;});
      
      // Step 1: Pre-evaluate with CLOCK = 0 to establish baseline prevClk state
      inputs.forEach(function(g,i){
        if(g.type==='CLOCK') { g.value=0; g.clockVal=0; }
        else { g.value=inVals[i]; }
      });
      if(typeof simulateCircuit==='function') simulateCircuit(gs,cs);

      // Step 2: Apply actual CLOCK values (0 or 1) and simulate to trigger edges
      inputs.forEach(function(g,i){
        g.value=inVals[i];
        if(g.type==='CLOCK') g.clockVal=inVals[i];
      });
    ...
    inputs.forEach(function(g,i){
      g.value=saved[i];
      if(g.type==='CLOCK') g.clockVal=saved[i];
    });
    ...
    wrap.querySelectorAll('tr.tt-row').forEach(function(row){
      row.addEventListener('click',function(){
        ...
        var gs2=(typeof gates!=='undefined')?gates:[];
        var inp=gs2.filter(function(g){return g.type==='INPUT'||g.type==='CLOCK';});
        if(!inp.length) return;
        var nn=inp.length;
        inp.forEach(function(g,i){
          var v = (mask>>(nn-1-i))&1;
          g.value=v;
          if(g.type==='CLOCK') g.clockVal=v;
        });
