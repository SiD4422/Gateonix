...
window.LFExport = (function() {
  ...
  function buildExportTruthTable() {
    var inputs  = gates.filter(function(g){ return g.type==='INPUT'||g.type==='CLOCK'; });
    ...
    // Save current state of ALL components
    var stateSnapshot = gates.map(function(g){
      return { q: g.q, nq: g.nq, prevClk: g.prevClk, clockVal: g.clockVal };
    });

    var rows = [];
    for (var mask=0; mask < (1<<n); mask++) {
      // Restore state for independent row evaluation
      ...
      // Step 1: Pre-evaluate with CLOCK = 0
      inputs.forEach(function(g,i){
        if(g.type==='CLOCK') { g.value=0; g.clockVal=0; }
        else { g.value=inVals[i]; }
      });
      if(typeof simulateCircuit==='function') simulateCircuit(gates, connections);

      // Step 2: Apply actual CLOCK values (0 or 1) to trigger edges
      inputs.forEach(function(g,i){
        g.value=inVals[i];
