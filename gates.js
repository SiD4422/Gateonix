...
function buildTruthTable(type) {
  ...
  if (!def || def.inputs < 1 || def.inputs > 4) return null;
  const n = def.inputs;
...
function renderTruthTable(type) {
  ...
  const outLabels = {
    'HADD':['S','C'],'FADD':['S','Co'],
    'COMP1':['>','=','<'],
    'SR_FF':['Q','Q̄'],'D_FF':['Q','Q̄'],'JK_FF':['Q','Q̄'],'T_FF':['Q','Q̄'],
  };
  const n = rows[0].ins.length;
  const m = rows[0].outs.length;
  const iL = inLabels[type] || Array.from({length:n}, (_,i)=>String.fromCharCode(65+i));
  const oL = outLabels[type] || (m===1?['Out']:Array.from({length:m}, (_,i)=>'Y'+i));
